...
def generate_with_citation(
    ...
    if not api_key:
        return {
            "answer": "❌ Thiếu API key. Vui lòng đặt GEMINI_API_KEY hoặc OPENROUTER_API_KEY trong file .env",
            "sources": chunks,
            "retrieval_source": chunks[0].get("source", "hybrid") if chunks else "none",
        }

    # Nếu dùng Gemini API Key thì trỏ về Google OpenAI-compatible endpoint
    if os.getenv("GEMINI_API_KEY"):
        base_url = "https://generativelanguage.googleapis.com/v1beta/openai/"
        actual_model = "gemini-1.5-flash" # fallback
        
        # Tự động fetch model khả dụng mới nhất từ Google AI Studio (chống lỗi model cũ)
        import requests
        try:
            resp = requests.get(f"https://generativelanguage.googleapis.com/v1beta/models?key={os.getenv('GEMINI_API_KEY')}", timeout=5)
            if resp.status_code == 200:
                ...
                # Sắp xếp để lấy model có version cao nhất (ví dụ gemini-3.5-flash > gemini-2.5-flash)
                if flash_models:
                    flash_models.sort()
                    actual_model = flash_models[-1]
        except Exception:
            pass
    ...
    # Step 5: Gọi LLM qua OpenAI-compatible API
    client = OpenAI(
        api_key=api_key,
        base_url=base_url,
    )
    ...
    try:
        ...
        # Nếu Google Gemini bị lỗi (429 Quota Exceeded do khu vực hoặc hết hạn ngạch)
        # Tự động Fallback sang OpenRouter bằng model miễn phí
