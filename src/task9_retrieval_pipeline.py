...
SCORE_THRESHOLD = 0.48  # Cosine score < 0.48 → không có chunk liên quan → fallback PageIndex
DEFAULT_TOP_K = 5
...
def retrieve(
    ...
    # Lấy top_k*2 để có đủ candidates cho bước reranking
    dense_results = semantic_search(query, top_k=top_k * 2)
    sparse_results = lexical_search(query, top_k=top_k * 2)

    # Step 2: Merge bằng RRF (Reciprocal Rank Fusion)
    # RRF chỉ dùng thứ hạng, không dùng điểm số → tránh lệch thang Cosine vs BM25
    merged = rerank_rrf([dense_results, sparse_results], top_k=top_k * 2)
    for item in merged:
        item["source"] = "hybrid"

    # Step 3: Rerank kết quả đã merge
    if use_reranking and merged:
        final_results = rerank(query, merged, top_k=top_k, method=RERANK_METHOD)
    else:
        final_results = merged[:top_k]

    # Step 4: Quyết định fallback dùng ĐIỂM COSINE GỐC — KHÔNG dùng điểm RRF
    # ⚠ RRF score luôn ≈ 1/(60+1) ≈ 0.016 bất kể câu hỏi có liên quan hay không
    # → Phải dùng dense_results[0]["score"] (cosine similarity [0,1]) để so ngưỡng
    # Ngưỡng 0.48: score < 0.48 → ChromaDB không có chunk nào thực sự liên quan
    #              → đưa chunk rác cho LLM sẽ bị hallucination → fallback PageIndex
    best_score = dense_results[0]["score"] if dense_results else 0.0
    if best_score < score_threshold:
        print(f"  [WARNING] Cosine score ({best_score:.3f}) < threshold ({score_threshold}) -> PageIndex fallback")
        try:
            ...
            print(f"  [WARNING] PageIndex fallback lỗi: {e} -> trả về hybrid results")
